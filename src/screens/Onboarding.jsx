...
export default function Onboarding({ onComplete }) {
  ...
  const canNext = step === 0 ? name.trim().length > 0 : step === 2 ? areas.length > 0 : true
    ...
    <div className="onboard">
      ...
      {step === 0 && (
        <>
          ...
          <p className="onboard-desc">Jede Legende braucht einen Namen. Alter und Gewicht sind optional — sie helfen dir später deinen Fortschritt einzuordnen.</p>
      ...
      {step === 1 && (
        <>
          ...
          <p className="onboard-desc">Was willst du erreichen? Schreib es auf — du wirst es jeden Tag sehen, wenn die Motivation mal fehlt.</p>
          <div className="onboard-fields">
            <textarea
              className="input"
      ...
      {step === 2 && (
        <>
          ...
          <p className="onboard-desc">Welche täglichen Ziele willst du angehen? Du kannst später jederzeit eigene erstellen.</p>
          <div className="chip-grid">
            {GOAL_AREAS.map(area => (
              <button key={area.id}
                className={`chip ${areas.includes(area.id) ? 'selected' : ''}`}
      ...
      <div className="onboard-footer" style={{ display: 'flex', gap: 10 }}>
        ...
        >
          {step < 2 ? 'Weiter →' : 'Abenteuer starten! 🔥'}
        </button>
      </div>
    </div>
  )
